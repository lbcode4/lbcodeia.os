# Calendário de Conteúdo — Junho/2026
Gerado em 03/06/2026 | 9 posts de feed + 3 sequências de stories | Mix: 42% carrossel · 33% reels · 25% stories

## Resumo do mês
- **Foco principal:** conseguir o 1º cliente — conteúdo de autoridade + prova de que automação com IA gera ROI (gargalo: nenhuma base ainda).
- **Cadência:** 2 posts de feed/semana (1 carrossel + 1 reel) + 1 sequência de stories/semana.
- **Campanhas ativas:** nenhuma (100% orgânico, sem ads ainda).
- **Datas especiais:** 12/jun Dia dos Namorados (oportunidade venda PME) · 24/jun São João / Festas Juninas (forte no Pará — ângulo local + founder).
- **Meta de alcance:** construir feed de autoridade antes de rodar tráfego pago. Direcionar tráfego pro diagnóstico grátis 15min (oferta de entrada).
- **Tom:** formal, autoridade, foco em ROI. Sem gírias, pouco emoji.
- **Visual:** dark + neon roxo→ciano (ver `identidade/design-guide.md`).

### Distribuição RETINA do mês (pilares reais do framework)
| Pilar | Posts | Papel |
|-------|-------|-------|
| **A** Autoridade | 2 | Prova que domina IA + dev sob medida |
| **N** Níveis de consciência | 3 | Problema → LBCode.IA como solução (conteúdo de venda) |
| **T** Transformação | 2 | Antes/depois — leva PME de A → B |
| **E** Engajamento | 1 | Alcance / curiosidade |
| **R** Relacionamento | 1 | Founder solo + Santarém (conexão) |
| **I** Interação | nos stories | Enquetes / perguntas pra ativar audiência |

---

## Semana 1 (03/jun — 07/jun) · parcial (começa hoje, qua)

| Data | Dia | Pilar | Tema sugerido | Gancho (GCC) | Formato | Skill | Status |
|------|-----|-------|---------------|--------------|---------|-------|--------|
| 04/jun | Qui | A | "Software sob medida vs sistema pronto: por que 'código 100% seu' economiza dinheiro" | Contraintuitivo — "O problema da sua empresa NÃO é falta de sistema. É depender do sistema dos outros." | Carrossel | `/conteudo-carrossel` | ⬜ |
| 06/jun | Sáb | N | "Sua empresa perde cliente às 22h? Atendimento que não dorme" | Pergunta — "Quantos clientes você perde fora do horário comercial?" | Reel | `/conteudo-reels` | ⬜ |

## Semana 2 (08/jun — 14/jun)

| Data | Dia | Pilar | Tema sugerido | Gancho (GCC) | Formato | Skill | Status |
|------|-----|-------|---------------|--------------|---------|-------|--------|
| 09/jun | Ter | T | "De planilha manual a dashboard em tempo real: o antes e depois de uma PME" | História — "Ela tomava decisão por feeling. Hoje decide por número." | Carrossel | `/conteudo-carrossel` | ⬜ |
| 11/jun | Qui | N | "Dia dos Namorados: a PME que automatiza o WhatsApp vende o dobro" (data: 12/jun) | Segmentado — "Dono de loja: leia antes do Dia dos Namorados." | Reel | `/conteudo-reels` | ⬜ |
| 12/jun | Sex | I | Stories — enquete "Qual seu maior gargalo hoje?" (atendimento / agenda / dados) + CTA diagnóstico | Pergunta direta na enquete | Stories | `/conteudo-stories` | ⬜ |

## Semana 3 (15/jun — 21/jun)

| Data | Dia | Pilar | Tema sugerido | Gancho (GCC) | Formato | Skill | Status |
|------|-----|-------|---------------|--------------|---------|-------|--------|
| 16/jun | Ter | A | "5 processos que toda PME ainda faz na mão (e a IA resolve em semanas)" | Contraintuitivo — "Você não precisa de mais funcionário. Precisa parar de fazer o que a IA já faz." | Carrossel | `/conteudo-carrossel` | ⬜ |
| 18/jun | Qui | E | "O que é 'código 100% seu' — e por que isso muda tudo pro seu negócio" | Pergunta — "Você sabe de quem é o sistema que faz sua empresa rodar?" | Reel | `/conteudo-reels` | ⬜ |
| 19/jun | Sex | I | Stories — bastidores "Construindo a LBCode.IA com IA (Claude Code)" + caixa de perguntas | História nos stories | Stories | `/conteudo-stories` | ⬜ |

## Semana 4 (22/jun — 28/jun)

| Data | Dia | Pilar | Tema sugerido | Gancho (GCC) | Formato | Skill | Status |
|------|-----|-------|---------------|--------------|---------|-------|--------|
| 23/jun | Ter | R | "Por que construí a LBCode.IA aqui em Santarém" (ângulo São João / local — 24/jun) | História — "Não saí do Pará pra empreender. Empreendi de dentro dele." | Carrossel | `/conteudo-carrossel` | ⬜ |
| 25/jun | Qui | T | "Agendamento inteligente: como uma agenda para de furar e gerar prejuízo" | Pergunta — "Quanto sua empresa perde por mês com agenda desorganizada?" | Reel | `/conteudo-reels` | ⬜ |
| 26/jun | Sex | I | Stories — sequência "Diagnóstico grátis 15min: como funciona" + CTA WhatsApp/Calendly | CTA único nos stories | Stories | `/conteudo-stories` | ⬜ |

## Semana 5 (29/jun — 30/jun) · parcial

| Data | Dia | Pilar | Tema sugerido | Gancho (GCC) | Formato | Skill | Status |
|------|-----|-------|---------------|--------------|---------|-------|--------|
| 30/jun | Ter | N | "Diagnóstico grátis de 15min: descubra se vale automatizar seu processo" | Segmentado — "PME que fatura na mão: esse diagnóstico é pra você." | Carrossel | `/conteudo-carrossel` | ⬜ |

---

## Banco de temas reserva
> Usar quando faltar inspiração ou precisar substituir um post planejado.

### Autoridade (A)
- "ROI em 2-3 meses: a conta que mostra quando automação se paga"
- "MVP em 2-6 semanas: por que entregamos 5x mais rápido (com IA)"
- "Os 50+ módulos da LBCode.IA — o que dá pra automatizar no seu negócio"

### Níveis de consciência (N)
- "60% dos clientes que não recebem resposta imediata vão pra concorrência"
- "Chatbot de IA no WhatsApp: atende, qualifica e marca reunião sozinho"
- "Decisão por feeling x decisão por dado: o custo invisível"

### Transformação (T)
- "Antes: agenda no caderno. Depois: sincronização em tempo real sem furo"
- "De 'respondo quando dá' a 'atendimento 24h' — sem contratar ninguém"

### Engajamento (E)
- "3 sinais de que seu negócio já deveria estar automatizado"
- "Mito: 'automação é só pra empresa grande'"

### Relacionamento (R)
- "Solo, com IA: como uma pessoa entrega o que antes precisava de equipe"
- "Por que não vendo sistema pronto — e sim solução sob medida"

### Interação (I)
- Enquete: "O que mais consome o tempo da sua equipe hoje?"
- Caixa de perguntas: "Manda seu processo manual — eu digo se dá pra automatizar"

---

## Checklist de publicação semanal
- [ ] Posts da semana criados (usar skills da coluna)
- [ ] Legendas revisadas — tom formal/autoridade, sem gírias, pouco emoji
- [ ] Visual conferido — dark + neon roxo→ciano (design-guide)
- [ ] Stories programados ou publicados
- [ ] Horário de pico respeitado (terça a quinta, 18h-20h — ajustar posts de fim de semana)
- [ ] CTA único e claro (diagnóstico grátis / WhatsApp / Calendly)
- [ ] Engajamento dos posts anteriores monitorado
