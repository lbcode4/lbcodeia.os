...
description: >
  ...
  /venda-dossie), gera roteiro de 10 pontos com pontos de conexão personalizados,
  ...
  "abordagem fria", "captar leads B2B", ou /venda-prospectar.
---

# /venda-prospectar — Fórmula PLANO + roteiro 10 pontos
...
- **Skill `/venda-dossie`** — levanta dossiê de cada lead
- **Skill `/conteudo-auditoria-insta`** (Posicionamento — auditar próprio IG do gestor)
...
1. Rodar `/conteudo-auditoria-insta` no perfil do GESTOR (não do cliente)
...
- Chamar `/venda-dossie` com nome+URL+handle
...
- Análise do GBP atual (chamar `/google-meu-negocio` diagnóstico)
- Análise do perfil IG (chamar `/conteudo-auditoria-insta` diagnóstico)
...
marketing/prospeccao/<nicho>-<YYYY-MM-DD>/
  ...
  dossies/                 ← 1 por lead priorizado (gerado por /venda-dossie)
    <slug-1>.md
    <slug-2>.md
    ...
