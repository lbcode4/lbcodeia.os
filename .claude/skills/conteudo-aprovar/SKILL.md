...
description: >
  ...
  commit + deploy automático (Netlify/Vercel), posta no IG + FB via Meta API. Uso: /conteudo-aprovar <slug>
  após conteúdo estar pronto em /conteudo-publicar. Tudo é versionado + automatizado.
  Use quando o usuário disser "aprovar post X", "publicar o tema Y", "/conteudo-aprovar <slug>".
---

# /conteudo-aprovar — Aprovação + Publicação automática
...
**Chamada:** `/conteudo-aprovar como-conservar-produto`
...
- Conteúdo deve vir de `/conteudo-publicar` (se vier avulso, avisar)
...
- Tom já calibrado em `/conteudo-publicar`
- Outputs vão pro histórico (`/sistema-salvar`)
- Próxima execução rodeia `/meta-relatorio` pra medir performance
