...
# /negocio-mapear-rotinas — De tarefa repetida pra skill própria
...
- Se `/conteudo-carrossel` já cobre criação de posts → oferecer parametrizar em vez de criar nova
- Se `/google-seo` já cobre pesquisa de nicho → mesma coisa
- Se `/meta-relatorio` já faz análise de campanha → mesma coisa
...
- Entrega outputs prontos pra versionagem via `/sistema-salvar`
