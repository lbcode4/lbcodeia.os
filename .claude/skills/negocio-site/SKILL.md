...
description: >
  ...
  (GCC + RETINA) e renderiza preview. Use quando o usuário pedir "site", "landing page",
  "página de vendas", "criar site", "LP", "página de captura", ou /negocio-site.
---

# /negocio-site — Sites e landing pages
...
- **Geração de imagem (se precisar de fotos/heros):** Gemini nano-banana 2 via `scripts/gerar-imagem-gemini.js` (default), OpenAI gpt-image-1 via `scripts/gerar-imagem.js` (fallback). Mesmas regras do `/conteudo-carrossel`.
...
Heros, fundos, mockups → mesmas regras do `/conteudo-carrossel`:
...
- Landing/campanha → "Quer que eu prepare a campanha de tráfego pra essa LP?" (`/meta-campanha-whatsapp` ou `/google-ads`)
- Qualquer → "Quer a versão de conteúdo (carrossel/reels) divulgando esse lançamento?" (`/conteudo-carrossel`)
