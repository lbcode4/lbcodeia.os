...
description: >
  ...
  resumo executivo num HTML só. Mais amplo que /lb-meta-dashboard (que foca hierarquia + funil).
  Resolve a conta em _memoria/contas-ads.md. Use quando o usuário pedir "dashboard completo meta",
  "relatório completo meta", "pago vs orgânico", "evolução de seguidores", ou /lb-meta-completo.
---

# /lb-meta-completo — Dashboard Completo Meta + Instagram (live API)
...
- **Credencial:** `integracoes/credentials/meta.env` (validar com `/lb-ads-conectar`)
...
## Erros
- Cliente sem IG User ID → este dashboard precisa do orgânico; avisar e pedir cadastro via `/lb-ads-conectar`.
