...
# /sistema-instalar — Setup Rápido do LBCode.IA

Primeiro comando pós-clone. Descobre essencial em 8-10min. Skills específicas
aprofundam depois quando user chamar (`/google-ads` faz briefing detalhado de ads,
`/google-seo` pesquisa de palavras-chave, etc).
...
1. **Sempre registrar @handle** — várias skills usam depois (`/conteudo-auditoria-insta`, 
   `/meta-campanha-seguidores`, legendas, dossiê).
...
3. Se vier vazio: registrar só @ e **não travar** — auditoria completa é `/conteudo-auditoria-insta`
   com prints.
4. **Não fazer auditoria aqui.** Oferecer depois:
   > "Quer que eu rode auditoria completa do perfil depois? (`/conteudo-auditoria-insta`)"
...
- Frequência conteúdo → qual skill primeiro (`/conteudo-carrossel` vs `/google-seo`)
...
**Por que:** Insumo pra RETINA (diferencial). Copy em `/conteudo-carrossel`, `/google-seo`.
...
**Por que:** Define se roda `/conteudo-carrossel` ou `/google-seo` primeiro. Determina velocidade.
...
*Registro:* Tarefa 1.
**Por que:** Candidata `/negocio-mapear-rotinas`.
...
**Por que:** KPI pra `/meta-relatorio`, `/google-seo`, `/venda-prospectar`. ROI real.
...
**Por que:** Skills de visual (`/conteudo-carrossel`, `/negocio-site`) usam depois.
...
Tarefa repetitiva: [P15] (candidata /negocio-mapear-rotinas)
Objetivo + Métrica: [P16] (ex: 80 agendamentos/mês, ROAS 4x)
Tráfego status: [P9-P10]
Prioridade #1: [P18]
```

**Crítico:** P16 é métrica real pra `/meta-relatorio`, `/google-seo`, `/venda-prospectar` depois.
...
> - `/sistema-salvar` commit + push automático
>
> **Primeira skill:**
...
> Depois de resolvido, roda `/negocio-mapear-rotinas` — viro skill automática.
>
> **Positioning:**
