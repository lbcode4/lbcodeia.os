...
describe("buildPrompt", () => {
  it("inclui nome da skill, cliente e briefing", () => {
    const p = buildPrompt("lb-meta-copy", "Empresa Teste", "anúncio de tênis", "text");
  ...
  it("injeta contrato JSON quando mode=data", () => {
    ...
    const p = buildPrompt("lb-meta-diagnostico", "Empresa Teste", "", "data", contract);
    expect(p).toContain("```json");
    expect(p).toContain('"score"');
  });

  it("não injeta JSON quando mode=text", () => {
    const p = buildPrompt("lb-meta-copy", "Empresa Teste", "briefing", "text");
