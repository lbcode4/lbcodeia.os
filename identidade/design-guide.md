# Identidade Visual

> Marca de quem executa tecnologia. Skills de conteúdo, carrossel, post e campanhas
> leem esse arquivo antes de criar qualquer visual.

---

## Status da identidade
Parcial — identidade visual existe mas não está totalmente documentada aqui.
Atualizar com logo, paleta e fontes quando disponível.

## Filosofia da marca
Tecnologia que gera resultado real. Visual minimalista, orientado a dados.
Transmite precisão, competência técnica e clareza — sem exageros.
Azul como cor principal (referência do site oferta.lbcodeia.com.br).

## Cores
- Fundo: #07070F
- Roxo neon: #A24BFF
- Ciano neon: #29C5FF
- Texto principal: #FFFFFF
- Texto secundário: #C9C9D6

## Tipografia
- Título: Playfair Display
- Corpo: Poppins

## Elementos
- Minimalismo
- Orientado a dados (gráficos, números, dashboards)
- Sem elementos decorativos desnecessários
- Linguagem visual direta (problema → solução)

## Galeria de referência

Nenhuma peça produzida ainda. Quando a skill visual gerar a primeira referência,
catalogar aqui seguindo `ref-<formato>-<conceito>.png`:
- `<formato>`: `post` (1:1) · `slide` (4:5) · `card` · `logo`
- `<conceito>`: tema/layout (ex: `servico`, `problema`, `ciclo`)

---

## Quando evoluir

Se a marca mudar:
1. Edita esse arquivo com as cores/tipografia novas
2. Skills carregam automaticamente na próxima execução
3. Documentação fica versionada
