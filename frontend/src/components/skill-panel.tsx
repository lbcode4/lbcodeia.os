...
export function SkillPanel({ skillId }: { skillId: string }) {
  ...
  const isOnboarding = skillId === "lb-sistema-onboarding";
  ...
  const iniciar = () => {
    if ((!isOnboarding && !cliente) || running) return;
    const parts: string[] = [];
        ...
        <Card className="lg:col-span-2 space-y-4 self-start">
          ...
          {!hasTurns ? (
            <>
                ...
                <textarea
                  ...
                  placeholder={
                    isCarrossel
                      ...
                      : isOnboarding
                      ? "Cole o link do site/Instagram (opcional) — ou deixe vazio e responda as perguntas da entrevista…"
              ...
              <Button onClick={iniciar} disabled={running || (!isOnboarding && !cliente)}>
                {running ? (
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto space-y-4 pr-1">
            {turns.length === 0 && !status && (
              <p className="text-[13px] text-muted-foreground">
                {isOnboarding ? "Clique em Executar pra começar a entrevista." : "Escolha o cliente e clique em Executar."}
              </p>
            )}
